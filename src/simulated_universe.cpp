...
void divideStrings(const char *num1, const char *num2, char *quotient) {
    if (strcmp(num2, "0") == 0) {
        _strcpy_s(quotient, sizeof(quotient), "error");
        return;
    }

    char remainder[MAX_SIZE] = "";
    _strcpy_s(remainder, sizeof(remainder), num1);
...
void moduloStrings(const char *num1, const char *num2, char *result) {
    if (strcmp(num2, "0") == 0) {
        _strcpy_s(result, sizeof(result), "error");
        return;
    }
    ...
    _strcpy_s(result, sizeof(result), remainder);
}
...
char* evaluate(const char* tokens) {
    ...
    if (!values.empty()) {
        ...
        _strcpy_s(finalResult, strlen(values.top().c_str()) + 1, values.top().c_str());
        return finalResult;
    }
